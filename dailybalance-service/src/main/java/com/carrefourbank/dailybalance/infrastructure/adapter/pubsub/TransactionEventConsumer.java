    ...
    private void processEvent(TransactionEventEnvelope envelope) throws Exception {
        ...
        switch (eventType) {
            case "transaction-created" -> {
                ...
                dailyBalanceService.applyTransaction(envelope.eventId(), data.transactionId(), data.date(), amount, type);
            }
            case "transaction-reversed" -> {
                ...
                TransactionType type = TransactionType.valueOf(data.type());
                dailyBalanceService.applyTransaction(envelope.eventId(), data.reversalTransactionId(), data.date(), money, type);
            }
